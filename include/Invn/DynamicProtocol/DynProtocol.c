...
static int DynProtocol_doProcess(DynProtocol_t * self)
{
	...
	self->decode_state_machine.state = self->decode_state_machine.PROTOCOL_STATE_IDLE;
...
int DynProtocol_processPktByte(DynProtocol_t * self, uint8_t rcvByte)
{
	...
	case self->decode_state_machine.PROTOCOL_STATE_GID:
	{
		self->decode_state_machine.event_type = (rcvByte & EVENT_TYPE_MASK);
		self->decode_state_machine.group_id   = (rcvByte & ~EVENT_TYPE_MASK);
		self->decode_state_machine.state      = self->decode_state_machine.PROTOCOL_STATE_CID;
	...
	case self->decode_state_machine.PROTOCOL_STATE_CID:
	{
		int rc;

		self->decode_state_machine.cmd_id     = rcvByte;
		self->decode_state_machine.state      = self->decode_state_machine.PROTOCOL_STATE_PAYLOAD;
	...
	case self->decode_state_machine.PROTOCOL_STATE_PAYLOAD:
	{
		int rc;
