		...
		DynProTransportEvent_cb event_cb, void * cookie)
{
	...
	self->rx_sm_state     = self->RECEIVER_STATE_IDLE;
	self->use_tx_dma        = 0;
}
...
int DynProTransportUart_rxProcessByte(DynProTransportUart_t * self, uint8_t rcv_byte)
{
	...
	case self->RECEIVER_STATE_IDLE:
		if(rcv_byte == SYNC_BYTE_0) {
			self->rx_sm_state = self->RECEIVER_STATE_SYNC_1;
		}
		else {
	...
	case self->RECEIVER_STATE_SYNC_1:
		if(rcv_byte == SYNC_BYTE_1) {
			self->rx_sm_state = self->RECEIVER_STATE_SIZE_BYTE_0;
		}
		else {
